...
export async function getAllCVEYearsAsJSON(deeplyPopulate = false) {
  ...
  export async function getProductByProductID(productID, deeplyPopulate) {
    await dbConnect();
    let product = {}
    ...
    if(deeplyPopulate){
      ...
      product = await CVEProduct.findById(productID)
    }
  ...
  export async function getAllCVEs(){
    await dbConnect();
    let cves = await CVE.find()
    let parsed =  JSON.parse(JSON.stringify(cves));
    return parsed;
  }